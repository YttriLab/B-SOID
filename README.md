...
```
### Step III
#### &nbsp;&nbsp;&nbsp;&nbsp; `Option 1`: Manual criteria for a rough but fast analyses (If you are interested in only one of the 7 behaviors)
```matlab
[g_label,g_num,perc_unk] = bsoid_fast(data,pix_cm); % data, pixel/cm
```
#### &nbsp;&nbsp;&nbsp;&nbsp; `Option 2`: Unsupervised grouping of action space (This can uncover behaviors that perhaps coincide better with latent variables) 

### Step IV (Only for `Option 2`)
```matlab
