...
%   Contact ahsu2@andrew.cmu.edu
    ...
    for it = 1:cv_it
        acc(it) = length(find((label_test(idx_samp((it-1)*btchsz+1:it*btchsz))-labels(idx_samp((it-1)*btchsz+1:it*btchsz)))==0))/length(labels(idx_samp((it-1)*btchsz+1:it*btchsz)));
    end
    CV_amean = mean(acc);
