function [feats,tsne_feats,grp,llh,bsoid_fig] = bsoid_gmm(data,fps,comp,smth_hstry,smth_futr,kclass,it)
...
%   Contact ahsu2@andrew.cmu.edu
    ...
    if nargin < 5
        smth_hstry = round(0.05/(1/100))-1;
        smth_futr = round(0.05/(1/100))-1;
    end
    if nargin < 6
        kclass = 30;
    end
    ...
    for n = 1:length(feats)
        ...
        feats_10fps{n} = feats1;
        if comp == 1
            feats_10fps_all = cat(2,feats_10fps_all,feats1);
            feats_all = cat(2,feats_all,feats{n});
        else
        %% For reproducibility
        ...
        tsne_feats = tsne(feats_10fps{n}(:,1:end)','Standardize',true,'Perplexity',50,'NumDimensions',3); % Refer to MATLAB tsne function for arguments
        %% Run a Gaussian Mixture Model Expectation Maximization to group the t-SNE clusters
        X = tsne_feats'; k = kclass; 
        [grp{n},model{n},llh{n}] = expmaxGMM(X,k,it);
        fprintf('TADA! \n');
    ...
    if comp == 1
        ...
        %% Run t-Distributed Stochastic Neighbor Embedding (t-SNE)
        tsne_feats = tsne(feats_10fps_all(:,1:end)','Standardize',true,'Perplexity',50,'NumDimensions',3); % Refer to MATLAB tsne function for arguments
        X = tsne_feats'; k = kclass; 
        [grp,model,llh,maxll] = expmaxGMM(X,k,it);
        fprintf('TADA! \n');
