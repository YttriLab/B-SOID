...
%   Contact ahsu2@andrew.cmu.edu
    ...
    for n = 1:length(feats)
        ...
        if comp == 1
            ...
            f_10fps{n} = feats1;
            if length(f_10fps{n}) < 15000
                p = 50;
            elseif length(f_10fps{n}) >= 15000 && length(f_10fps{n}) < 30000
                p = 100;   
            elseif length(f_10fps{n}) >= 30000 && length(f_10fps{n}) < 45000
                p = 150;
            elseif length(f_10fps{n}) >= 45000 && length(f_10fps{n}) < 60000
                p = 200;
            elseif length(f_10fps{n}) >= 60000 && length(f_10fps{n}) < 75000
                p = 250;
            elseif length(f_10fps{n}) >= 75000 && length(f_10fps{n}) < 90000
                p = 300;
            elseif length(f_10fps{n}) >= 90000 && length(f_10fps{n}) < 105000
                p = 350;
            elseif length(f_10fps{n}) >= 105000 && length(f_10fps{n}) < 120000
                p = 400;    
            else
    ...
    if comp == 1
        if length(f_10fps) < 15000
            p = 50;
        elseif length(f_10fps) >= 15000 && length(f_10fps) < 30000
            p = 100;   
        elseif length(f_10fps) >= 30000 && length(f_10fps) < 45000
            p = 150;
        elseif length(f_10fps) >= 45000 && length(f_10fps) < 60000
            p = 200;
        elseif length(f_10fps) >= 60000 && length(f_10fps) < 75000
            p = 250;
        elseif length(f_10fps) >= 75000 && length(f_10fps) < 90000
            p = 300;
        elseif length(f_10fps) >= 90000 && length(f_10fps) < 105000
            p = 350;
        elseif length(f_10fps) >= 105000 && length(f_10fps) < 120000
            p = 400;    
        else
            error(sprintf('Reduce the dataset, might take too long to run... \nHowever, if you think you know what you are doing, edit bsoid_gmm and set perplexity value yourself :D \n'))
        end
